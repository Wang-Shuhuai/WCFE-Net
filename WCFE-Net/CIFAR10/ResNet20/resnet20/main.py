...
writer = SummaryWriter(os.path.join(args.results_dir, 'runs/bb_Jump_Prelu_WDF0B1e-5_concat_Nalpha_L40S200ema95grad_seed1_bn'))
...
def main():
    ...
    if args.optimizer == 'sgd':
        ...
        optimizer = torch.optim.SGD([{'params':fp_params, 'weight_decay':0,'initial_lr': args.lr},
                                     {'params': bin_params, 'weight_decay':1e-5,'initial_lr': args.lr}],
                                    lr=args.lr,
                                    momentum=args.momentum,
                                    weight_decay= 1e-5)
