...
conv_count = 0
writer = SummaryWriter('./results/runs/init')
...
#             init.kaiming_normal_(m.weight)
#         else:
#             torch.nn.init.uniform_(m.weight, -0.002,0.002)
#
#         writer.add_histogram("INIT", m.weight, conv_count)
...
class BasicBlock_1w1a(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride != 1 or in_planes != planes:
            self.shortcut = nn.Sequential(
                        nn.AvgPool2d((2,2)),
                        LambdaLayer(lambda x:x.repeat(1,2,1,1)))
            # self.shortcut = nn.Sequential(
            #             nn.AvgPool2d((2,2)),
            #             LambdaLayer(lambda x:
            #             F.pad(x, (0, 0, 0, 0, pad, pad), "constant", 0)))  #这种padding会大量丢失信息，而被padding的数据还要进行计算，造成数据浪费
