...
writer = SummaryWriter(os.path.join(args.results_dir, 'runs/Res18archRes20_Jump_Prelu_concat_WDF5e-5B5e-5_Nalpha_L40S200gradema_seed2_initBn10'))
# writer = SummaryWriter('/root/tf-logs/AbWb_Jump_kaiming_Prelu_NSqrt_WDF0B1e-4_concat_Nalpha_L40S100grad_th_seed1')
...
class BinOp():
    ...
    def calculate_flip(self, ep, lr):
        if self.current_ep != ep:
            ...
            for index in range(self.num_of_params):
                self.clip_th += self.clip_th_tmp[index]
                self.clip_th_tmp[index] = 0

            self.clip_th =  self.clip_th / 195 / 18
            writer.add_scalar('PerEpoch total_th', self.clip_th, ep - 1)
            self.flip_count = 0
        ...
        for index in range(self.num_of_params):
            ...
            writer.add_scalar('gradnormal/{}'.format(index), sth, total_iter)
            ...
            self.target_modules[index].data = self.target_modules[index].data.clamp(-40*gne_correct, 40*gne_correct)

            self.flip_count += (self.saved_params[index].sign() != self.target_modules[index].data.sign()).sum()
            self.clip_th_tmp[index] += sth
...
def main():
    ...
    if args.optimizer == 'sgd':
        ...
        optimizer = torch.optim.SGD([{'params':fp_params, 'weight_decay':5e-5,'initial_lr': args.lr},
                                     {'params': bin_params, 'weight_decay':5e-5,'initial_lr': args.lr}],
                                    lr=args.lr,
                                    momentum=args.momentum,
                                    weight_decay= 5e-5)
